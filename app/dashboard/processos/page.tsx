...
function ProcessosContent() {
  ...
  // 5. Buscar no CNJ (Integração - Modal de criação)
  const handleBuscaCNJ = async () => {
    ...
    if (res.found && res.dados) {
      ...
      showToast(`Processo localizado no ${res.dados.tribunal}! Campos preenchidos.`, "success");
    } else {
      showToast(res.error || "Processo não encontrado na base do CNJ.", "error");
    }
  };

  // 6. Sincronizar CNJ (Processo já existente)
  const handleSincronizarCNJ = async (processoId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Aguarda um pouco e recarrega os dados para pegar as atualizações
        setTimeout(() => {
          carregarDados();
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {processos.map((proc) => {
            ...
            const isSyncing = sincronizando[proc.id] || false;

            return (
              <div
                key={proc.id}
                className={`relative bg-white p-6 rounded-3xl border transition-all group ${isArquivado
              ...
              >
                ...
                {!isArquivado && proc.dataPrazo && (
                  <div className={`absolute left-0 top-6 bottom-6 w-1 rounded-r-full transition-colors ${statusPrazo === 'vencido' ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' :
                ...
                <div className="flex justify-between items-start mb-4 pl-3">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className={`appearance-none cursor-pointer pl-3 pr-6 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border outline-none transition-colors ${proc.prioridade === 'Urgente' ? 'bg-red-50 text-red-600 border-red-100 hover:bg-red-100' :
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    {!isArquivado && (
                      <>
                        {/* Botão Sincronizar CNJ */}
                        <button
                          onClick={() => handleSincronizarCNJ(proc.id)}
                          disabled={isSyncing}
                          className="p-2 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors disabled:opacity-50"
                ...
                <div className="pl-3 flex justify-between items-end mb-4 mt-2">
                  ...
                  {/* Badge de Prazo */}
                  {!isArquivado && proc.dataPrazo && (
                    <span className={`text-xs font-bold flex items-center gap-1.5 px-2 py-1 rounded-lg border ${statusPrazo === 'vencido' ? 'text-red-600 bg-red-50 border-red-100' :
                ...
                {/* Informações do CNJ (quando sincronizado) */}
                {proc.tribunal && (
                  <div className="pl-3 mb-3 flex flex-wrap gap-1.5">
                    <span className="text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-md">
                      {proc.tribunal}
                    </span>
                    {proc.orgaoJulgador && (
                      <span className="text-[10px] font-medium bg-slate-50 text-slate-500 border border-slate-100 px-2 py-0.5 rounded-md truncate max-w-[200px]">
                        {proc.orgaoJulgador}
                      </span>
                    )}
                    {proc.sincronizadoEm && (
                      <span className="text-[10px] text-slate-400 flex items-center gap-1 ml-auto">
                        <RefreshCw size={10} />
                        {format(new Date(proc.sincronizadoEm), "dd/MM HH:mm")}
                      </span>
                    )}
                  </div>
                )}
                ...
                <div className="pl-3 pt-4 border-t border-slate-50 flex justify-between items-center">
                  ...
                  <div className={`text-xs font-bold px-3 py-1.5 rounded-lg border ${isArquivado
